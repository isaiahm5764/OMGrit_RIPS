            ...
            braid_TriStatus status)
{
   ...
   double  t, tprev, tnext, dt, dx;
   double *utmp, *utmp2, *W_old, *r1, *r2, *r3, *r4 /*r4 corresponds to residual for u^n-1*/;
   int mspace = (app->mspace);
   double nu = (app->nu);
   /*double *li = (app->li);
   double *ai = (app->ai);*/
   ...
   double *dW, *dU, *dU_old, *dV, *storage1, *storage2, *storage3, *storage4;

   //printf("Above delta initizlizers\n");
   /*Initialize the delta vectors*/
   vec_create(mspace, &dW);
   vec_create(mspace, &dU);
   vec_create(mspace, &dU_old);
   vec_create(mspace, &dV);
   for(int i=0; i<mspace; i++) dW[i]=0.0;
   for(int i=0; i<mspace; i++) dV[i]=0.0;
   for(int i=0; i<mspace; i++) dU[i]=0.0;
   for(int i=0; i<mspace; i++) dU_old[i]=0.0;
   ...
   /* Start GS iteration */
   int num_c_iters=(app->num_c_iters);
   double norm = 0.0;
   for(int i = 0; i<num_c_iters; i++)
   {
    norm = 0.0;
    if(uleft != NULL)
    {
      //printf("Above uleft vects\n");
      vec_copy(mspace, (uleft->values)[2], W_old);      

      //printf("Here 1\n");
      /* Update dU_old */
      vec_copy(mspace, dW, utmp);
      vec_axpy(mspace, 1.0, r4, utmp);
      apply_C_inverse(dt,dx,nu,mspace, W_old ,utmp);
      vec_copy(mspace, utmp, dU_old);
      ...
      apply_Aadjoint(dt, dx, nu, mspace, utmp);
      apply_B(dt, mspace, nu, utmp2, storage1);
      vec_scale(mspace, g(dt,dx), utmp2);
      vec_axpy(mspace, 1.0, utmp2, utmp);
      vec_axpy(mspace, -1.0, r1, utmp);
      vec_scale(mspace, -1.0, utmp);
      apply_C_inverse(dt,dx,nu,mspace,storage3,utmp);
      vec_copy(mspace, utmp, dU);

      //printf("Here 3\n");
      /* Update dV */
      vec_copy(mspace, dU, utmp);
      vec_copy(mspace, dU, utmp2);
      apply_A(dt, dx, nu, mspace, utmp);
      apply_B(dt, mspace, nu,utmp2, storage1);
      vec_scale(mspace, g(dt,dx), utmp2);
      vec_axpy(mspace, 1.0, utmp2, utmp);
      vec_axpy(mspace, -1.0, r3, utmp);
      vec_axpy(mspace, -1.0, dU_old, utmp);
      vec_scale(mspace, 1.0/dt, utmp);
      vec_copy(mspace, utmp, dV);       

      //printf("Here 4\n");
      /* Update dW */
      vec_copy(mspace, dV, utmp);
      vec_scale(mspace, alpha*dx*dt, utmp);
      vec_axpy(mspace, -1.0, r2, utmp);
      vec_scale(mspace, 1.0/dt, utmp);
      vec_copy(mspace, utmp, dW);     

      //printf("Here 5\n");
      /* Compute dU_old part of residual */
      vec_copy(mspace, dU_old, utmp);
      vec_copy(mspace, dW, utmp2);
      apply_C(dt, dx, nu, mspace, (uleft->values[2]), utmp);
      vec_axpy(mspace, -1.0, utmp2, utmp);
      vec_axpy(mspace, -1.0, r4, utmp);
      for(int j = 0; j < mspace; j++)
      {
        norm = norm + (utmp[j] * utmp[j]);
      }
    }

    //printf("Outside uleft conditional\n");
    if(uleft == NULL)
    {
      //printf("Uleft is null:\n");
      /* Update dU */
      vec_copy(mspace, dW, utmp);
      vec_copy(mspace, dW, utmp2);
      apply_Aadjoint(dt, dx, nu, mspace, utmp);
      ...
      vec_axpy(mspace, 1.0, utmp2, utmp);
      vec_axpy(mspace, -1.0, r3, utmp);
      vec_axpy(mspace, -1.0, dU_old, utmp); /* What dU_old value should be here? */
      vec_scale(mspace, 1.0/dt, utmp);
      vec_copy(mspace, utmp, dV);  
    ...
    apply_B(dt, mspace, nu, utmp2, storage1);
    vec_scale(mspace, g(dt,dx), utmp2);
    vec_axpy(mspace, 1.0, utmp, utmp2);

    vec_copy(mspace, dU, utmp);
    apply_C(dt, dx, nu, mspace, storage3, utmp);
    vec_axpy(mspace, 1.0, utmp2, utmp);
    vec_axpy(mspace, -1.0, r1, utmp);
    for(int j = 0; j < mspace; j++)
    {
    ...
    /* Compute dW residual  */ 
    vec_copy(mspace, dW, utmp);
    vec_copy(mspace, dV, utmp2);
    vec_scale(mspace, -dt, utmp);
    vec_scale(mspace, alpha*dx*dt, utmp2);
    vec_axpy(mspace, 1.0, utmp2, utmp);
    vec_axpy(mspace, -1.0, r2, utmp);
    for(int j = 0; j < mspace; j++)
    {
    ...
    vec_scale(mspace, g(dt,dx), utmp2);
    vec_axpy(mspace, 1.0, utmp2, utmp);
    vec_axpy(mspace, -dt, dV, utmp);
    vec_axpy(mspace, -1.0, r3, utmp);
    if(uleft != NULL)               /* What should dU_old be? */
    {
      vec_axpy(mspace, -1.0, dU_old, utmp);
    }
    else
   ...
   vec_axpy(mspace, -1.0, dU_old, storage4);
   ...
   vec_copy(mspace, storage4, u->values[3]);
...
main(int argc, char *argv[])
{
   ...
   double      alpha, nu;
   int         max_levels, min_coarse, nrelax, nrelaxc, cfactor, maxiter, num_c_iters;
   ...
   while (arg_index < argc)
   {
      if ( strcmp(argv[arg_index], "-help") == 0 )
      {
         ...
         printf("  -nci <nci>              : Number of c iterations \n");
         exit(1);
      }
      ...
      else if ( strcmp(argv[arg_index], "-nci") == 0 )
      {
         arg_index++;
         num_c_iters = atof(argv[arg_index++]);
      }      
      else
      {
