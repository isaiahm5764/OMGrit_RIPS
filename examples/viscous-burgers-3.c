               ...
               braid_TriStatus status)
{
   ...
   utmp[0] = (0.5-b(dt,dx,nu))*utmp[1]+utmp[0]*(2*b(dt,dx,nu))+g(dt,dx)*utmp[0]*(utmp[1]);
   for(int i = 1; i <= mspace-2; i++)
   {
    utmp[i] = (0.5-b(dt,dx,nu))*(utmp[i-1]+utmp[i+1])+utmp[i]*(2*b(dt,dx,nu))+g(dt,dx)*utmp[i]*(utmp[i+1]-utmp[i-1]);
   }
   utmp[mspace-1] = (0.5-b(dt,dx,nu))*(utmp[mspace-2])+utmp[mspace-1]*(2*b(dt,dx,nu))+g(dt,dx)*utmp[mspace-1]*(-utmp[mspace-2]);
            ...
            braid_TriStatus status)
{
   ...
   rtmp[0] = (-1/(2*b(dt,dx,nu)+g(dt,dx)*(utmp[1])))*rtmp[0]; 
   /*printf("1 entry divided by %lf \n", (1+2*b(dt,dx,nu)+g(dt,dx)*(utmp[1])));*/
   for(int i = 1; i <= mspace-2; i++)
   {
    rtmp[i] = (-1/(2*b(dt,dx,nu)+g(dt,dx)*(utmp[i+1]-utmp[i-1])))*rtmp[i];
    /*printf("%d entry divided by %lf \n", i+1,(1+2*b(dt,dx,nu)+g(dt,dx)*(utmp[i+1]-utmp[i-1])));*/
   }
   rtmp[mspace-1] = (-1/(2*b(dt,dx,nu)+g(dt,dx)*(-utmp[mspace-2])))*rtmp[mspace-1]; 
   ...
   if (access_level > 0)
   {
      ...
      * optimization case. */

      {
         ...
         sprintf(filename, "%s.%03d", "out/viscous-burgers-3.w", (app->myid));
         file = fopen(filename, "w");
