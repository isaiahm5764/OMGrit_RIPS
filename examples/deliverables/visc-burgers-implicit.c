...
{
   double  t, tprev, tnext, dt, dx;
   double *utmp, *r1, *r2, *r3 /*r4 corresponds to residual for u^n-1*/;
   int mspace = (app->mspace);
   double nu = (app->nu);
   //double *li = (app->li);
   ///double *ai = (app->ai);
   ...
   /*solve for deltaW*/
    ...
    apply_C_inverse(dt,dx,nu,mspace,storage3,dW);
    vec_copy(mspace, dW, utmp);
    apply_B_transpose(dt,mspace,nu,dW,storage1);
    vec_scale(mspace,g(dt,dx),dW);
   ...
   //apply c_tilde inverse
    ...
    apply_B_transpose_inverse(dt,dx,nu,mspace,storage1,dU);
...
main(int argc, char *argv[])
{
  ...
  dx = 1/((double)(mspace+1));
   ...
   char    filename[255];
   FILE   *file;
   sprintf(filename, "%s.%d.%d.%f.%f.%d", "visc-burgers-implicit.conv", ntime,mspace,nu,alpha,max_levels);
  ...
  if (access_level > 0)
  {
    ...
    sprintf(filename, "%s.%03d", "visc-burgers-implicit.out.u", (app->myid));
    file = fopen(filename, "w");
    ...
    sprintf(filename1, "%s.%03d", "visc-burgers-implicit.out.u0", (app->myid));
    ...
    sprintf(filename, "%s.%03d", "visc-burgers-implicit.out.v", (app->myid));
    file = fopen(filename, "w");
    for (i = 0; i < (app->npoints); i++)
    {
      double ***w = (app->w);
    ...
    sprintf(filename, "%s.%03d", "visc-burgers-implicit.out.w", (app->myid));
    file = fopen(filename, "w");
    for (i = 0; i < (app->npoints); i++)
    {
      double ***w = (app->w);
  ...
  printf("Total Run Time: %f s \n", time);
  {
    char    filename[255];
    FILE   *file;
    sprintf(filename, "%s.%d", "visc-burgers-implicit.time", ntime);
    file = fopen(filename, "w");
    fprintf(file, "%f", time);
    fflush(file);
    fclose(file);
