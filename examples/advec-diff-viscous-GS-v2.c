...
void 
add_Gamma(int mspace, double dt, double dx, double *u_n, double *v)
{
   double g = g(dt,dx);
   v[0]+=u_n[0]*u_n[1];
               ...
               braid_TriStatus status)
{
   ...
   if(index>=2*(ntime-1)/3)
   {
      ...
      if(index == 2*(ntime-1)/3)
      {
         ...
         add_Gamma(mspace, dt, dx, utmp, rtmp);
   ...
   if((ntime-1)/3<=index && index<2*(ntime-1)/3)
   {
      ...
      */
      ...
      if(index == (ntime-1)/3)
      {
         ...
         add_Gamma(mspace, dt, dx, u_state[0], vtmp);
      ...
      else
      {
         ...
         add_Gamma(mspace, dt, dx, u_state[index-(ntime-1)/3], vtmp);
