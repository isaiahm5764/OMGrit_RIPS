...
my_TriSolve(braid_App       app,
            ...
            braid_TriStatus status)
{
   double  t, tprev, tnext, dt, dx, g, b, omega;
   double *utmp, *uleft_tmp, *uold;
   int mspace = (app->mspace);
   double nu = (app->nu);
   double *u0 = (app->U0);
   ...
   utmp=(u->values);
   /*vec_copy(mspace, (uleft->values), uleft_tmp);*/
   /*printf("To here 2b\n");*/

   if (uleft==NULL)
   {
   	utmp[0]=(u0[0]+b*(utmp[1]))/(1+2*b+g*utmp[1]);
   	for (int i = 1; i < mspace-2; i++)
   	{
   		utmp[i]=(u0[i]+b*(utmp[i-1]+utmp[i+1]))/(1+2*b+g*(utmp[i+1]-utmp[i-1]));
   	}
   	utmp[mspace-1]=(u0[mspace-1]+b*(utmp[mspace-1]))/(1+2*b-g*(utmp[mspace-1]));
   }
   /*printf("Got to 4\n");*/
   ...
   {
   	/*printf("To here 1c\n");*/
   	uleft_tmp=(uleft->values);
   	/*printf("To here 2c\n");*/
   	utmp[0]=(uleft_tmp[0]+b*(utmp[1]))/(1+2*b+g*utmp[1]);
   	for (int i = 1; i < mspace-2; i++)
   	{
   		utmp[i]=(uleft_tmp[i]+b*(utmp[i-1]+utmp[i+1]))/(1+2*b+g*(utmp[i+1]-utmp[i-1]));
   	}
   	utmp[mspace-1]=(uleft_tmp[mspace-1]+b*(utmp[mspace-1]))/(1+2*b-g*(utmp[mspace-1]));   	
   }
   /*printf("Got to 5\n");*/
   vec_axpy(mspace, -1.0, uold, utmp);
   vec_scale(mspace, omega, utmp);
   vec_axpy(mspace, 1.0, uold, utmp);
...
main(int argc, char *argv[])
{
   ...
   if (access_level > 0)
   {

      {
         ...
         sprintf(filename, "%s.%03d", "out/viscous-burgers-GS.w", (app->myid));
         file = fopen(filename, "w");
