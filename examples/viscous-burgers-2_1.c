 ...
 *               replaced by previous time solution for i+1 and i-1
 *
               ...
               braid_TriStatus status)
{
   ...
   if (uleft != NULL)
   {
      ...
      rtmp[0] = rtmp[0]+g(dt,dx)*utmp[0]*(u2tmp[1]);
      for(int i = 1; i <= mspace-2; i++)
      {
        rtmp[i] = rtmp[i]+g(dt,dx)*utmp[i]*(u2tmp[i+1]-u2tmp[i-1]);
      }
      rtmp[mspace-1] = rtmp[mspace-1]+g(dt,dx)*utmp[mspace-1]*(-u2tmp[mspace-2]);
   }
   else if (uleft == NULL)
   {
      vec_copy(mspace, (r->values), utmp);
      rtmp[0] = rtmp[0]+g(dt,dx)*utmp[0]*(u0[1]);
      for(int i = 1; i <= mspace-2; i++)
      {
        rtmp[i] = rtmp[i]+g(dt,dx)*utmp[i]*(u0[i+1]-u0[i-1]);
      }
      rtmp[mspace-1] = rtmp[mspace-1]+g(dt,dx)*utmp[mspace-1]*(-u0[mspace-2]);
   }
...
if (uleft != NULL)
  {
   ...
   rtmp[0] = (-1/(1+2*b(dt,dx,nu)+g(dt,dx)*(u2tmp[1])))*rtmp[0]; 
   /*printf("1 entry divided by %lf \n", (1+2*b(dt,dx,nu)+g(dt,dx)*(utmp[1])));*/
   for(int i = 1; i <= mspace-2; i++)
   {
    rtmp[i] = (-1/(1+2*b(dt,dx,nu)+g(dt,dx)*(u2tmp[i+1]-u2tmp[i-1])))*rtmp[i];
    /*printf("%d entry divided by %lf \n", i+1,(1+2*b(dt,dx,nu)+g(dt,dx)*(utmp[i+1]-utmp[i-1])));*/
   }
   rtmp[mspace-1] = (-1/(1+2*b(dt,dx,nu)+g(dt,dx)*(-u2tmp[mspace-2])))*rtmp[mspace-1]; 
   /*printf*/
 }
   ...
   if (access_level > 0)
   {
      ...
      * optimization case. */

      {
         ...
         sprintf(filename, "%s.%03d", "out/viscous-burgers-2_1.w", (app->myid));
         file = fopen(filename, "w");
