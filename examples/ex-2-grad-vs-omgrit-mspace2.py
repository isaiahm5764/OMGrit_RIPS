...
count=0
for n in ntime:
    with open('out/advec-diff-imp.time.' + str(n)+".1"+".20") as f:
        lines = f.readlines()
    advec_diff_imp_1p[count] = lines[0]
    with open('out/advec-diff-imp.time.' + str(n)+".8"+".20") as f:
        lines = f.readlines()
    advec_diff_imp_8p[count] = lines[0]    
    with open('out/advec-grad-serial.time.' + str(n)+".20") as f:
        lines = f.readlines()
