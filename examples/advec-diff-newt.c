 ...
 ***********************************************************************EHEADER*/
 ...
 /**
 * Example:       advec-diff-newt.c
 *
 * Interface:     C
 * 
 * Requires:      only C-language support     
 *
 * Compile with:  make advec-diff-newt
 *
...
main(int argc, char *argv[])
{
  ...
  if (access_level > 0)
  {
    ...
    sprintf(filename, "%s.%03d", "out/advec-diff-newt.out.u", (app->myid));
    ...
    sprintf(filename1, "%s.%03d", "out/advec-diff-newt.out.u0", (app->myid));
    ...
    sprintf(filename, "%s.%03d", "out/advec-diff-newt.out.v", (app->myid));
    file = fopen(filename, "w");
    for (i = 0; i < (app->npoints); i++)
    {
      double ***w = (app->w);
    ...
    sprintf(filename, "%s.%03d", "out/advec-diff-newt.out.w", (app->myid));
    file = fopen(filename, "w");
    for (i = 0; i < (app->npoints); i++)
    {
      double ***w = (app->w);
  ...
  printf("Total Run Time: %f s \n", time);
  {
    char    filename[255];
    FILE   *file;
    sprintf(filename, "%s.%d", "out/advec-diff-newt.time", ntime);
    file = fopen(filename, "w");
    fprintf(file, "%f", time);
    fflush(file);
    fclose(file);
