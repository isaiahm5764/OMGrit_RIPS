...
main(int argc, char *argv[])
{
   ...
   */
   ...
   do
   {
      ...
      /* Solve L*w^(k+1)=Uu^(k) */
      vec_copy(mspace, w[ntime-1], w_old[ntime-1]);
   ...
   /**********************PRINT TIME OUT**********************/
   {
      ...
      FILE   *file;
      */
      sprintf(filename, "%s.%d", "out/block_gj_ex1.time", maxiter);
      file = fopen(filename, "w");
      fprintf(file, "%f", time);
      fflush(file);
      fclose(file);
   ...
   /**********************PRINT TOTAL RES OUT**********************/
   {
      ...
      FILE   *file;
      */
      sprintf(filename, "%s.%d", "out/block_gj_ex1.res", maxiter);
      file = fopen(filename, "w");
      fprintf(file, "%f", norm);
      fflush(file);
      fclose(file);
   ...
   {
      sprintf(filename, "%s.%d.%f", "out/block_gj_ex1.conv", ntime, gamma);
      file = fopen(filename, "w");
