...
with open('out/advec-diff-imp-full-res.out.u.000') as f:
        lines = f.readlines()
for line in lines:
    line = line[6:]
    split = line.split(',')
...
with open('out/advec-diff-imp-full-res.u0.000') as f:
    lines = f.readlines()
split = lines[0].split(',')
count2=1
state_vec[0,0] = 0
...
print(state_vec)
with open('out/advec-diff-imp-full-res.out.w.000') as f:
    lines = f.readlines()
count = 0
for line in lines:
    line = line[6:]
...
with open('out/advec-diff-imp-full-res.out.v.000') as f:
    lines = f.readlines()
count = 0
for line in lines:
    line = line[6:]
