...
count=0
for n in ntime:
    with open('out/advec-diff-imp-full-res.res.' + str(n)) as f:
        lines = f.readlines()
    ex_2_omgrit[count] = lines[0]
    with open('out/block_gs.res.' + str(n)) as f:
        lines = f.readlines()
    ex_2_gs[count] = lines[0]
    with open('out/block_gj.res.' + str(n)) as f:
        lines = f.readlines()
    ex_2_gj[count] = lines[0]
...
mpl.title('Residual for Linear Advection Diffusion Problem')
